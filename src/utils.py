...
def visual_ebd(sr_embedding, tg_embedding):
    # 创建一个TSNE对象
    tsne = TSNE(n_components=2)
    data = torch.cat((sr_embedding, tg_embedding), dim=0)
    data = data.to('cpu').detach().numpy()
    # data1 = sr_embedding.to('cpu').detach().numpy()
    # data2 = tg_embedding.to('cpu').detach().numpy()

    embedded_data = tsne.fit_transform(data)
    # embedded_data2 = tsne.fit_transform(data2)
    embedded_data1 = embedded_data[:15000, :]
    embedded_data2 = embedded_data[15000:, :]
    # del data1, data2
    # 可视化嵌入
    plt.scatter(embedded_data1[:, 0], embedded_data1[:, 1], s=2, c='goldenrod')
    plt.scatter(embedded_data2[:, 0], embedded_data2[:, 1], s=2, c='steelblue')

    del embedded_data1, embedded_data2
    plt.title('GAEA')
    # 保存结果
    plt.savefig('2GAEA.png')
