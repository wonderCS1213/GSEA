...
class KGData():
        ...
        with open(file_name, 'r', encoding='utf-8') as f:
            ...
            for item in data:
                ...
                if item[0] not in ent2id:
                    ent2id[item[0]] = ent_id   # ent:id
                    id2ent[ent_id] = item[0]   # id:ent
    ...
    def find_potential_alignment(self, sr_embedding, tg_embedding, threshold=0.8):
        ...
        for leftid, rightid in enumerate(left2right):
            if right2left[rightid] == leftid and leftid not in self.mapped_train_pair[:,0] and rightid not in self.mapped_train_pair[:,1]: # 双向匹配
                similarity = 1 - sim_mat_left2right[leftid, rightid]
