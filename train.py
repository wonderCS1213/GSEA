...
parser.add_argument("--model", type=str, default='gsea', help="the model's name", choices=['gsea','gaea', 'gat', 'gcn', 'svd-gaea', 'svdgcn'])
parser.add_argument("--task", type=str, default='en_fr_15k', help="the alignment task name",
                    choices=['en_fr_15k', 'en_de_15k', 'd_w_15k', 'd_y_15k', 'en_fr_100k', 'en_de_100k', 'd_w_100k',
                             'd_y_100k'])
...
parser.add_argument("--eval_metric", type=str, default="csls", help="the distance metric of entity pairs",
                    choices=["cityblock", "euclidean", "csls", "inner"])
parser.add_argument('--neg_samples_size', type=int, default=25, help="number of negative samples25")
parser.add_argument('--neg_iter', type=int, default=10, help="re-calculate epoch of negative samples")
parser.add_argument("--neg_metric", type=str, default="csls",
...
parser.add_argument('--truncated_epsilon', type=float, default=0.88, help='the epsilon of truncated negative sampling')
...
parser.add_argument('--aug_balance', type=float, default=55, help='the hyper-parameter of consistency loss default=55')
parser.add_argument('--aug_iter', type=int, default=10)
parser.add_argument('--q', default=5, type=int, help='rank default=5')
...
kgdata = KGData(model=args.model, task=args.task, device=device, neg_samples_size=args.neg_samples_size, fold=args.fold,
                train_ratio=args.train_ratio * 0.1, val=args.val, direct=args.direct)
kgdata.data_summary()
...
if args.model == "gcn":
    model = GCN(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=kgdata.tensor_adj1,
                adj_tg=kgdata.tensor_adj2, u_mul_s_sr=u_mul_s1, vt_sr=svd_v1.T, u_mul_s_tg=u_mul_s2, vt_tg=svd_v2.T,
                embedding_dim=args.ent_dim, dropout=args.dropout, layer=args.layer)
elif args.model == "gat":
    ...
    model = GAEA(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=kgdata.tensor_adj1,
                 adj_tg=kgdata.tensor_adj2, args=args)
elif args.model == "svdgcn":

    model = SVDGCN(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=adj1,
                   adj_tg=adj2, u_mul_s_sr=u_mul_s1, vt_sr=svd_v1.T, u_mul_s_tg=u_mul_s2, vt_tg=svd_v2.T,
                   embedding_dim=args.ent_dim, dropout=args.dropout, layer=args.layer, rel_num=kgdata.rel_num,
                   rel_adj_sr=kgdata.rel_adj1, rel_adj_tg=kgdata.rel_adj2, device=device, args=args)
elif args.model == "gsea":

    model = GSEA(num_sr=kgdata.kg1_ent_num, num_tg=kgdata.kg2_ent_num, adj_sr=kgdata.tensor_adj1,
                   adj_tg=kgdata.tensor_adj2, u_mul_s_sr=u_mul_s1, vt_sr=svd_v1.T, u_mul_s_tg=u_mul_s2, vt_tg=svd_v2.T,
                   embedding_dim=args.ent_dim, dropout=args.dropout, layer=args.layer, rel_num=kgdata.rel_num,
                   rel_adj_sr=kgdata.rel_adj1, rel_adj_tg=kgdata.rel_adj2, device=device, args=args)
...
def evaluate(test_pair, k, eval_metric, phase="test", model_path=None):
    ...
    # with open('embedding1.pkl', 'wb') as f0:
    #     pickle.dump(sr_embedding, f0)
    #
    # with open('embedding2.pkl', 'wb') as f1:
    #     pickle.dump(tg_embedding, f1)
    #
    # visual_ebd(sr_embedding, tg_embedding)
    # sr_embedding, tg_embedding = model()
...
try:
    ...
    for e in range(args.epoch):
        ...
        for _, data in enumerate(train_loader):
            ...
            if args.model == "gaea":
                if args.pr != 0:
                    ...
                    aug_sr_embedding, aug_tg_embedding = model(aug_adj1, aug_adj2, phase="augment")
                    '''alignment loss'''
                ...
                sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding = model(input1=kgdata.tensor_adj1, input2=kgdata.tensor_adj2)
                loss = loss_fn(sr_embedding, tg_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left,
                               neg2_right, neg_samples_size=args.neg_samples_size, loss_norm=args.loss_norm,
                               pos_margin=args.pos_margin, neg_margin=args.neg_margin, neg_param=args.neg_param)
            elif args.model in ["svd-gaea", "svdgcn", 'gsea']:
                # sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding = model(input1=kgdata.tensor_adj1, input2=kgdata.tensor_adj2)
                # sr_embedding, tg_embedding, aug_sr_embedding, aug_tg_embedding = model(aug_rel_adj1, aug_rel_adj2)
                (sr_embedding, tg_embedding,
                 aug_sr_embedding, aug_tg_embedding) = model(u_mul_s1_ri=u_mul_s1_ri,
                                                             ...
                                                             svd_v2_ro_t=svd_v2_ro.T, phase="aug")
                loss = loss_fn(sr_embedding, tg_embedding, a1_align, a2_align, neg1_left, neg1_right, neg2_left,
                               neg2_right, neg_samples_size=args.neg_samples_size, loss_norm=args.loss_norm,
                               pos_margin=args.pos_margin, neg_margin=args.neg_margin, neg_param=args.neg_param)
        ...
        print(f"epoch: {e + 1}, loss: {round(loss.item(), 3)},"
              f" time: {round((time.time() - t_start), 2)}\n")

        '''validation phase'''
        if args.val and (e + 1) % args.val_iter == 0 and e > 0 and (e + 1) >= args.val_start:
            hit_1_score, hit_k_score, mrr, mean = evaluate(test_pair=kgdata.mapped_val_pair, k=args.k,
                                                           eval_metric=args.eval_metric, phase="val")
            if best_val < mrr:
                ...
                print(
                    f"[validation: epoch: {e + 1}, hit@1: {round(hit_1_score, 3)}, hit@{args.k}: {round(hit_k_score, 3)}, mrr is {round(mrr, 3)}]\n")
            else:
                bad_count += 1
        ...
        if args.il and (e + 1) >= args.il_start and (e + 1) % args.il_iter == 0 and e > 0 and e + 1 != args.epoch:
            new_train_pair = kgdata.find_potential_alignment(sr_embedding.detach().cpu().numpy(),
        ...
        if (e + 1) % args.neg_iter == 0 and e > 0 and e + 1 != args.epoch:
            neg1_left, neg1_right, neg2_left, neg2_right = kgdata.update_neg_sample(sr_embedding.detach().cpu().numpy(),
                                                                                    tg_embedding.detach().cpu().numpy(),
                                                                                    neg_samples_size=args.neg_samples_size,
                                                                                    eval_metric=args.neg_metric,
                                                                                    csls_k=args.csls_k, e=args.truncated_epsilon)
...
try:
    hit_1_score, hit_k_score, mrr, mean = evaluate(test_pair=kgdata.mapped_test_pair, k=args.k,
                                                   eval_metric=args.eval_metric, phase="test", model_path=model_path)
    print("----------------final score----------------\n")
    ...
    if args.result:
        ...
        with open(f'result/{args.model}_{args.task}.csv', 'a', encoding='utf-8') as file:
            file.write('\n')
            file.write(
                f"{args.model}, {args.fold}, {round(hit_1_score, 3)}, {round(hit_k_score, 3)}, {round(mrr, 3)}, {round(mean, 3)}, {args.ent_dim}, {args.rel_dim}, {args.lr}, {args.pr}, {args.aug_balance}, {args.eval_metric}, {args.neg_margin}")
        print("-------------------------------------------\n")
except KeyboardInterrupt:
    sys.exit()
